...
#데이터 파일 불러와 표로 만들기-------------------------------------------------
list.files()
data1 = 'INU바로미터 총학생회 여론조사 3주차.csv' %>% 
  read_csv( Encoding('cp949')) %>% 
  mutate(주차 ='1')
data2 = 'INU바로미터 총학생회 여론조사 3주차.csv' %>% 
  read_csv( Encoding('cp949')) %>% 
  mutate(주차 = '2')
names(data1)
...
name.data <- data2 %>%  # data숫자로 주차 바꾸기
  ...
  filter(as.numeric(X6) > 10000000 & as.numeric(X6) < 1000000000) %>% 
  filter(X2 < 135 | X2 > 141)
write.csv(name.data, "총학생회 여론조사 3주차.csv", fileEncoding = "cp949")
# 1. 설문조사 단과대 비율 확인하기
dan.data= name.data %>% 
  group_by(단과대학) %>%
  filter(!is.na(단과대학)) %>% 
...
dande = '자연과학대학' #삭제 원하는 단과대
how = 12 # 단과대 설문조사 추출 명 - 최종삭제인원
몇명제거 = data4 %>% 
  filter(단과대학 == dande) %>% 
...
sampled_data <- bind_rows(
  name.data %>% filter(학과 == "경제학과") %>% slice_sample(n = 5),
  name.data %>% filter(학과 == "행정학과") %>% slice_sample(n = 3),
  name.data %>% filter(학과 == "소비자학과") %>% slice_sample(n = 3),
  name.data %>% filter(학과 == "미디어커뮤니케이션학과") %>% slice_sample(n = 3),
  name.data %>% filter(학과 == "사회복지학과") %>% slice_sample(n = 2),
  name.data %>% filter(학과 == "화학과") %>% slice_sample(n = 8),
  name.data %>% filter(학과 == "법학부") %>% slice_sample(n = 5),
  name.data %>% filter(!학과 %in% c("경제학과", "행정학과", "소비자학과", "법학부", "미디어커뮤니케이션학과", "사회복지학과", "화학과"))
)
write.csv(sampled_data, "3주차_표본조정후.csv", fileEncoding = 'cp949')
# 전처리 후 단과대 비율 확인
...
투표율_표본조정 <- as.data.frame(t(voting_summary))
# 7-1. 누구에게 투표하려고 하십니까?
